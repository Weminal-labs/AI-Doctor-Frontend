...
export default function Header(props: HeaderProps) {
  const NavItem_Elements = React.useMemo(() => {
    return Object.keys(RouteNames).map(function (key: string, index: number) {
      if (index === 0 || RouteNames[key].canHideFromHeader) return;
        ...
        {!props.rightSide ? (
          <div className="flex items-center">
            <nav className="border-r px-3 hidden sm:block">
              <ul className="flex flex-row">{NavItem_Elements}</ul>
            </nav>
            <Button
              colorType="onFirst"
              buttonType="normal"
              className="flex p-2 me-3 rounded sm:hidden"
              onClick={() => alert("You clicked on menu")}
            >
              <span className="material-symbols-outlined text-background bg-on-background">
                more_vert
              </span>
            </Button>
          </div>
